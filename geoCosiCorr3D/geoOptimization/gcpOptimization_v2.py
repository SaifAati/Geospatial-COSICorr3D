...
class cGCPOptimization:
    ...
    def ingest(self):
        ...
        logging.info(f'input GCPs:{self.gcp_df.shape}')
        ...
        if self.sat_model_name == SATELLITE_MODELS.RSM:
            if self.sensor not in GEOCOSICORR3D_SENSORS_LIST:
                ...
                logging.info(f' {self.sat_model_name} sensor: {self.sensor}')
                self.sat_model = RSM.build_RSM(metadata_file=self.sat_model_metadata, sensor_name=self.sensor,
                                               debug=self.debug)
                logging.info(f' {self.sat_model_name} model: {self.sat_model}')
    ...
    def set_dem(self):
        ...
        if self.dem_info is not None:
            if self.dem_info.epsg_code != self.epsg:
                new_dem_path = geoRT.ReprojectRaster(input_raster_path=self.dem_path, o_prj=self.epsg)
    ...
    def correct_gcp(self, snr, dx, dy, gcp_patch, ew, ns):
        ...
        ## For the momenet we keep the same h -----> @SA: Future work
...
#     folder = '/home/cosicorr/0-WorkSpace/3-PycharmProjects/geoCosiCorr3D/geoCosiCorr3D/Tests/3-geoGCP_optimization_Test/Sample2'
...
#     dem_path = os.path.join(folder, 'rOrtho.TIF')
#     opt = cGCPOptimization(gcp_file_path=gcp_file, raw_img_path=raw_img_path, ref_ortho_path=ref_img_path,
#                            dem_path=)
#     pass
