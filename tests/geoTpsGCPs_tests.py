...
class TestGeoFunctions(unittest.TestCase):
    ...
    def setUpClass(cls):
        folder = os.path.join(SOFTWARE.PARENT_FOLDER.parent, "tests/test_dataset")
        cls.img1 = os.path.join(folder, "BASE_IMG.TIF")
        cls.img2 = os.path.join(folder, "TARGET_IMG.TIF")
        cls.match_file = os.path.join(folder, 'basemap_VS_SP2.pts')
        cls.raw_img_path = os.path.join(folder, 'SP2.TIF')
        cls.ref_img_path = os.path.join(folder, 'basemap.TIF')
        cls.dem_path = os.path.join(folder, 'DEM.TIF')

    def test_tp_to_gcps(self):
        with tempfile.TemporaryDirectory(dir=SOFTWARE.WKDIR, suffix='_test_gcp') as tmp_dir:
            tp_2_gcp = TPsTOGCPS(in_tp_file=self.match_file,
                                 base_img_path=self.raw_img_path,
        ...
        np.testing.assert_allclose(expec_array, gcp_array, rtol=1e-3,
                                   err_msg="Expected and actual GCP arrays do not match")

    def test_asift_tps(self):
        with tempfile.TemporaryDirectory(dir=SOFTWARE.WKDIR, suffix='_test_asift') as tmp_dir:
            tp_obj = cMicMacTp(ref_img_path=self.img1,
                               raw_img_path=self.img2,
                               scale_factor=1 / 6,
                               plot_tps=False,
                               o_dir=tmp_dir
                               )
            ...
            self.assertTrue(np.allclose(np.loadtxt(tp_obj.o_tp_path, comments=';'), expec_tps),
                            msg="Expected and actual ASIFT TPs do not match")
